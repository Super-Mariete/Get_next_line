...
/*   Updated: 2024/10/27 10:45:11 by made-ped         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	while (!(ft_strchr(buffer[fd], '\n')) && read_bytes != 0)
	{
		...
		if (read_bytes == -1)
		{
			buffer[fd] = free_and_null(line, buffer[fd]);
			return (NULL);
		}
		*(line + read_bytes) = '\0';
	...
	buffer[fd] = create_substring(buffer[fd]);
	return (line);
}
...
int	main (int	argc, char	**argv)
{
	...
	while (files > 0)
	{
		...
		while (i < argc - 1)
		{
			if (fd[i] != -1)
			{
				result = get_next_line(fd[i]);
				if (result)
				{
					printf ("\033[32mArchivo %d :\033[0m %s\n", i + 1, result);
					free (result);
				}
				else
				{
